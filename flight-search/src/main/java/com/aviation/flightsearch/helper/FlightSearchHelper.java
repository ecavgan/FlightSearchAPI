...
public class FlightSearchHelper {
	public static List<FlightDTO> findOneWayFlights(SearchRequest request, List<Flight> allFlights, ModelMapper modelMapper) {
		...
		depFlights = 
			allFlights
			.stream()
			.filter(f -> f.getDepartureAirport().getId() == reqDepartureAirport.getId() &&
						 f.getArrivalAirport().getId() == reqArrivalAirport.getId() &&
	...
	public static List<FlightDTO> findTwoWayFlights(SearchRequest request, List<Flight> allFlights, ModelMapper modelMapper) {
		...
		rtnFlights = 
			allFlights
			.stream()
			.filter(f -> f.getDepartureAirport().getId() == reqArrivalAirport.getId() &&
						 f.getArrivalAirport().getId() == reqDepartureAirport.getId() &&
